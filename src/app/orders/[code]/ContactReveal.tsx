  ...
  const reveal = async () => {
    ...
    try {
      ...
      if (!res.ok || !data || !Array.isArray(data.items)) {
        throw new Error("Thông tin chưa khớp — kiểm tra lại SĐT/email lúc đặt hàng");
      }
      setOrder(data);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Không xem được chi tiết");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="mt-space-sm w-full border-t border-outline-variant/20 pt-space-md">
      <p className="font-body-sm text-body-sm text-on-surface-variant/80">
        Chi tiết món chỉ hiện cho chủ đơn — nhập SĐT/email đã dùng lúc đặt hàng:
      </p>
      <div className="mt-space-xs flex flex-wrap justify-center gap-2">
        <input
          ...
          placeholder="SĐT hoặc email lúc đặt hàng"
        ...
        >
          {busy ? "Đang kiểm tra..." : "Xem chi tiết"}
        </button>
      </div>

...
describe("cookie helpers", () => {
  it("mặc định dev: SameSite=Lax, không Secure (đủ cho localhost khác port)", () => {
  ...
  it("prod khác domain: SameSite=None + Secure", () => {
    process.env.COOKIE_SAMESITE = "none";
    try {
      expect(sessionCookie("tok.abc")).toBe(
        `${SESSION_COOKIE}=tok.abc; Path=/; HttpOnly; SameSite=None; Secure; Max-Age=${SESSION_MAX_AGE}`
      );
    } finally {
      delete process.env.COOKIE_SAMESITE;
    }

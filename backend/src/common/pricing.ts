import { strapOptions } from './strap-options';

/**
 * Tỉ giá USD → VND (port từ frontend src/lib/pricing.ts).
 ...
 * VND suy ra theo USD_TO_VND.
 */
export function linePrice(usd: number, strap?: string | null): LinePrice {
  const delta = strapOptions.find((s) => s.label === strap)?.priceDeltaUsd ?? 0;
